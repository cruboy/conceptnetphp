...
<style> 
.pcs{width:100px; float:left; text-align:center;}
</style>
...
<div style="width:<? echo strlen($pDa['text'])*9+180;?>px;"><a onClick="$('#ftt'+theid).hide();theid=0;$('#ftt0').show();" 
onDblClick="theid=-1;   $.ajax({	url:'m/docp.php?cp=<?=$cpidd?>&ecdid=<?=$pDa['id']?>',
				type:'POST',
				data:$('#fttz').serialize(),
...
<div id="ftt0" style='display:none;background:url(content/templates/images/bg.gif)'>
<?php echo $pDa['id']; ?>
...
?>;$('#ftt<?=$value['aid']?>').show();" 
onDblClick="theid=-1;$.ajax({url:'m/docp.php?cp=<?=$cpidd?>&aid=<?=$value['aid']?>',type:'POST',
				data:$('#ft<?=$value[aid]?>').serialize(),
				success: function(data){ alert(data);}}); $('#ftt<?=$value['aid']?>').hide();">○</a><a href="?ep=<?php echo $value['id']; ?>" title="<?=$value['aid']?>:<?php echo $value['f1'].'+'.$value['f2'].'='.$value['f3']; ?>
:<?php echo $value['num_assertions']; ?>:<?php echo $value['relation_id'].".".$value['best_frame_id'].' '.$value['rela'].
":".$value['frame']; ?>:V<?php echo $value['visible']; ?>:C<?php echo $value['cruboy']; ?>"><?php echo $value['text'];
 ?></a><?php if($value['aurl'] !='' ): ?>
...
</div><div id="ftt<?=$value['aid']?>" style='display:none;z-index:660;background:url(content/templates/images/bg.gif) '>
